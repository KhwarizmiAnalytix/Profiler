...
profiler_report::profiler_report(const profiler::profiler_session& session)
    : was_active_(session.is_active()),
      start_time_snapshot_(session.session_start_time()),
      end_time_snapshot_(session.session_end_time()),
      scope_tree_snapshot_(session.build_scope_tree_shared())
{
    if (const auto* tracker = session.memory_tracker_ptr())
    {
        has_memory_stats_ = true;
        memory_snapshot_  = tracker->get_current_stats();
    }
    if (const auto* analyzer = session.statistical_analyzer_ptr())
    {
        has_statistics_                = true;
        timing_stats_snapshot_         = analyzer->calculate_all_timing_stats();
        memory_stats_by_name_snapshot_ = analyzer->calculate_all_memory_stats();
    }
    if (session.has_collected_xspace())
    {
        has_xspace_      = true;
        xspace_snapshot_ = session.collected_xspace();
    }
}
...
// Looks up the snapshot captured once at construction -- calculate_all_memory_stats()'s per-key
// value is computed identically to calculate_memory_stats(name), so this is exact, not approximate.
profiler::statistical_metrics profiler_report::scope_memory_stats(const std::string& scope_name) const
{
    if (!has_statistics_)
    {
        return {};
    }
    auto it = memory_stats_by_name_snapshot_.find(scope_name);
    if (it == memory_stats_by_name_snapshot_.end())
    {
        return {};
    }
    return it->second;
...
// Offline tabular summary from the snapshotted XSpace (same events Chrome/report use).
// Complements statistical_analyzer (online multi-sample mean/std).
std::string profiler_report::build_xspace_stats_summary() const
{
    if (!has_xspace_)
    {
        return {};
    ...
    const xplane* host = find_plane_with_name(xspace_snapshot_, kHostThreadsPlaneName);
    if (host == nullptr)
    {
        return {};
    }
        ...
        [&](const xline_visitor& line)
        {
            line.for_each_event(
                [&](const xevent_visitor& event)
                {
                    ...
                    {
                        const auto mem_stats = scope_memory_stats(name);
...
{
    auto const* root = scope_tree_snapshot_.get();
    ...
    ss << "{\n";
    ss << "  \"header\": {\n";
    ss << "    \"active\": " << (was_active_ ? "true" : "false") << ",\n";
    ss << "    \"scope_count\": " << snapshots.size() << ",\n";
    ss << "    \"max_depth\": " << compute_max_depth(snapshots) << ",\n";

    auto const start_time = start_time_snapshot_;
    auto const end_time   = end_time_snapshot_;
    ...
    ss << "  \"memory\": {\n";
    if (has_memory_stats_)
    {
        ss << "    \"current_bytes\": " << memory_snapshot_.current_usage_ << ",\n";
        ss << "    \"peak_bytes\": " << memory_snapshot_.peak_usage_ << ",\n";
        ss << "    \"total_allocated_bytes\": " << memory_snapshot_.total_allocated_ << ",\n";
        ss << "    \"total_deallocated_bytes\": " << memory_snapshot_.total_deallocated_ << "\n";
    }
    else
    {
        ss << "    \"enabled\": false\n";
    ...
    auto const thread_histogram =
        sort_map_by_value_desc(build_thread_histogram(xspace_snapshot_));
...
std::string profiler_report::generate_header_section() const
{
    ...
    ss << "Session active: " << (was_active_ ? "yes" : "no") << "\n";
...
std::string profiler_report::generate_summary_section() const
{
    ...
    ss << "Threads involved: " << build_thread_histogram(xspace_snapshot_).size()
       << "\n";

    if (has_memory_stats_)
    {
        ss << "Current memory: " << format_memory_size(memory_snapshot_.current_usage_) << "\n";
        ss << "Peak memory: " << format_memory_size(memory_snapshot_.peak_usage_) << "\n";
        ss << "Total allocated: " << format_memory_size(memory_snapshot_.total_allocated_) << "\n";
        ss << "Total deallocated: " << format_memory_size(memory_snapshot_.total_deallocated_)
           << "\n";
    }
    else
    {
...
std::string profiler_report::generate_memory_section() const
{
    ...
    std::vector<std::pair<std::string, profiler::statistical_metrics>> entries(
        memory_stats_by_name_snapshot_.begin(), memory_stats_by_name_snapshot_.end());
    std::sort(
        entries.begin(),
        entries.end(),
...
std::string profiler_report::generate_statistical_section() const
{
    ...
    {
        if (timing_stats_snapshot_.empty())
        {
        ...
        {
            size_t count = 0;
            for (const auto& entry : timing_stats_snapshot_)
            {
    ...
    const std::string xspace_stats = build_xspace_stats_summary();
...
std::string profiler_report::generate_hotspot_section() const
{
    ...
    const hotspot_report report(scope_tree_snapshot_);
    ...
    const profiler_scope_data& scope, std::stringstream& ss, int indent) const
{
    ...
    auto const        memory = scope_memory_stats(scope.name_);
